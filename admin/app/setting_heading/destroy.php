...
if ($_SERVER['REQUEST_METHOD'] == 'DELETE' && isset($_GET['id'])) {
  ...
  $check = $conn->query("SELECT ID FROM setting_headings WHERE ID = $id");
  if ($check->num_rows > 0) {
    $delete = $conn->query("DELETE FROM setting_headings WHERE ID = $id");
    if ($delete) {
      echo json_encode(['status' => 200, 'message' => 'setting_headings deleted successfully!']);
    } else {
      echo json_encode(['status' => 302, 'message' => 'Something went wrong!']);
    }
  } else {
    echo json_encode(['status' => 302, 'message' => 'setting_headings not exists!']);
  }
}
