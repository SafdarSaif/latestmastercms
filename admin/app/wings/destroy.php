...
if ($_SERVER['REQUEST_METHOD'] == 'DELETE' && isset($_GET['id'])) {
  ...
  $check = $conn->query("SELECT Media_File FROM wings WHERE ID = $id");
  if ($check->num_rows > 0) {
    $row = $check->fetch_assoc();
    $photoPath = $row['Media_File'];

    $delete = $conn->query("DELETE FROM wings WHERE ID = $id");
    if ($delete) {
      if ($photoPath && file_exists("../../" . ltrim($photoPath, "/"))) {
        unlink("../../" . ltrim($photoPath, "/"));
        echo json_encode(['status' => 200, 'message' => 'Wing deleted successfully, and associated photo removed!']);
      } else {
        echo json_encode(['status' => 200, 'message' => 'Wing deleted successfully, but no associated photo was found!']);
      }
    } else {
      echo json_encode(['status' => 302, 'message' => 'Something went wrong while deleting the page!']);
    }
  } else {
    echo json_encode(['status' => 302, 'message' => 'Wing does not exist!']);
  }
}
