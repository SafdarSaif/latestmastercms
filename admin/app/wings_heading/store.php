...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($category)  || empty($content) || empty($position)) {
        echo json_encode(['status' => 403, 'message' => 'All required fields must be filled out!']);
        exit();
    }

    // Check if name already exists
    $checkQuery = $conn->query("SELECT ID FROM wings_heading WHERE Name = '$name'");
    ...
    $query = "INSERT INTO wings_heading 
              (`Name`, `Slug`, `Category`, `Content`, `Meta_Title`, `Meta_Key`, `Meta_Description`, `Position`, `Created_At`) 
              VALUES ('$name', '$slug', '$category', '$content', '$meta_title', '$meta_key', '$meta_description', '$position', NOW())";
